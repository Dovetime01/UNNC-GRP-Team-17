...
## 27. 本轮新增进展：原生插件 UI 面板已接入

本轮已将原来的“按钮即执行测试逻辑”入口升级为原生 Slate 编辑器面板：

- 插件按钮现在会打开一个可停靠的 `Nomad Tab`
- 新增本地模型路径输入区
- 新增 `.gguf` 文件浏览按钮
- 新增 Prompt 多行输入区
- 新增状态提示区
...
当前 UI 目标是先建立一个稳定、可扩展的编辑器内交互壳层，为后续：
- 流式输出
- 推理中断
- 会话历史
- 参数调节
...
## 28. 本轮新增进展：UI 已接入异步单次推理流程

为了避免在按钮点击时直接阻塞主线程，当前 UI 已改为：

1. 在面板内填写模型路径与 Prompt
2. 点击“生成回复”后，在后台线程执行一次完整请求
3. 后台流程为：
	- `Initialize()`
	- `Generate()`
	- `Shutdown()`
...
1. 将模型生命周期从“单次请求”提升为“面板级驻留”
2. 在后台工作线程里维持长期存活的推理服务
3. 增加 token 级流式输出
4. 增加 Stop/Cancel 控制
5. 增加采样参数可视化配置